...
summary: Acceptez et abonnez-vous à une offre privée Marketplace pour la NetApp Console dans le portail Azure.  Configurez les informations d’identification Azure dans la console. 
---
...
Accepter et activer l'offre plusieurs fois peut entraîner une double facturation.

====
.Étapes
. Sélectionnez le lien de l’offre privée fourni par votre équipe commerciale NetApp et connectez-vous. Si aucun lien n’est disponible, connectez-vous directement au portail Azure.
. Recherchez ou sélectionnez *Marketplace* parmi les services Azure.
. Sur la page Marketplace, sélectionnez *Gestion des offres privées* dans la section *Gestion*.
. Sélectionnez *Voir + accepter* pour consulter les détails de l'offre privée.
+
...
. Confirmez et sélectionnez *Accepter l'offre privée*.
. Sélectionnez *Acheter* pour finaliser le processus d'achat.
+
[NOTE]
====
...
. Sélectionnez *Review and subscribe*, validez les informations, puis sélectionnez *Subscribe*.
. Une fois l'abonnement créé, sélectionnez *Configurer le compte maintenant*. Vous serez alors redirigé vers la NetApp Console pour finaliser la configuration.
+
[NOTE]
====
L'automatisation côté Azure peut prendre entre 30 secondes et 5 minutes pour se terminer. Si vous avez fermé la page avant de sélectionner *Configurer le compte maintenant*, retournez au service dans le portail Azure. L'abonnement s'affiche comme *Configuration en attente*. Sélectionnez-le pour poursuivre la configuration.
...
.. Sélectionnez les *Organizations* de NetApp Console qui doivent avoir accès à cet abonnement.
.. Vous pouvez activer l’option *Remplacer l’abonnement existant* pour remplacer automatiquement un abonnement Marketplace existant dans une organisation Console.
+
[NOTE]
====
*** Cette option de remplacement est limitée à une seule organisation Console.
...
Pour plus d'informations sur la facturation des abonnements marketplace et des licences directes, consultez link:concept-billing-preferences.html["Découvrez les préférences de facturation pour les licences et les abonnements."].
